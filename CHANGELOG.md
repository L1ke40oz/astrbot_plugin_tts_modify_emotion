...
### 新增
* 新增可视化配置界面，支持在 AstrBot 管理面板中直接编辑语音参数
* 新增基准参数设置（基准语速、基准音调、基准音量）
* 新增情绪强度系数配置，可控制情绪参数的影响强度
* 新增情绪参数模板列表，支持动态添加/删除/编辑情绪

### 修改
* 优化配置项显示，每个情绪配置块显示为 "显示名 (情绪名)" 格式
* 配置界面参数自动同步到 emotion_params.json，保持向后兼容

### 原因
* 降低用户配置门槛，避免手动编辑 JSON 文件
* 参考 elevenlabs_tts 插件的设计，提供一致的用户体验
* 保留 emotion_params.json 作为中间层，确保现有逻辑不变
